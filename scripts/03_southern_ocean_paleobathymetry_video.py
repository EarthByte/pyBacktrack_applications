#!/usr/bin/env python3
"""
Figure 3 video - 1 Myr south-pole orthographic animation of global
paleobathymetry.  South-pole companion to
`02a_global_paleobathymetry_video.py`: same per-Myr rendering pipeline
(Crameri `nuuk` + `grdgradient` hillshading + plate-model overlay),
just centred on the south pole.
...
Outputs (source-aware so Z22_mantle and Z22_PMag videos coexist):
    figures/output/southern_ocean_paleobathy/frames_<SOURCE>/frame_NNNN.png
    figures/output/fig03_southern_ocean_paleobathymetry_video_<SOURCE>.mp4
    figures/output/fig03_southern_ocean_paleobathymetry_video_<SOURCE>_forward.mp4

Usage:
    python3 03a_southern_ocean_paleobathymetry_video.py --source Z22_mantle
    python3 03a_southern_ocean_paleobathymetry_video.py --source Z22_PMag

    # Use the PALEO_BATHY_SOURCE set in config.py
    python3 03a_southern_ocean_paleobathymetry_video.py

    # Quick check: render every 5 Myr up to 100 Ma
    python3 03a_southern_ocean_paleobathymetry_video.py --time-step 5 --max-age 100
"""
import argparse
import os
import shutil
...
def _parse_args():
    p = argparse.ArgumentParser(
        description=("Render a 1 Myr south-pole-centred orthographic "
                     "paleobathymetry animation, then stitch frames into "
                     "MP4."),
    )
    p.add_argument(
        "--source",
...
# Lambert Azimuthal Equal-Area projection centred on the South Pole,
# with HORIZON_DEG=50 -> cap edge at lat -40 S.  Matches the static
# Fig 3 (03_southern_ocean_paleobathymetry.py) projection family.
SOUTHPOLE_LON = 0.0
SOUTHPOLE_LAT = -90.0
HORIZON_DEG   = 50.0
PROJ_WIDTH_CM = 14
...
OUT_BASE = os.path.join(OUTPUT_DIR, "southern_ocean_paleobathy")
...
def _banner():
    bar = "=" * 78
    print(bar)
    print(" SOUTH-POLE PALEOBATHYMETRY VIDEO -- run configuration")
    print(bar)
...
def _wipe_gradient_cache_if_forced():
    ...
    (`pbr.GRAD_DIR`, shared with the north-Atlantic video).
    gradient_for() keys its cache only on the integer age, so stale
    grad files produced under different HILLSHADE_* params silently
    survive a plain frame-only --force.
...
def render_all_frames(gplot):
    ...
    # Audit the cache up front so the run log shows what will be
    # reused vs regenerated -- the previous silent-skip behaviour was
    # confusing.
